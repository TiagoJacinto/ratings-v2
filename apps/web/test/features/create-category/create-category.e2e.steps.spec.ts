import { autoBindCreateCategorySteps, CategoryName } from "@repo/features";
import { CreateCategoryPageController } from "test/support/pages/create-category/controllers/create-category.page-controller";
import { PuppeteerCreateCategoryPageController } from "@test/support/pages/create-category/controllers/implementations/PuppeteerCreateCategoryPageController";
import { Driver } from "@primitivestack/frontend-testing-puppeteer";
import { ResponseConfig } from "@src/shared/api/client";
import { beforeAll } from "vitest";
import { MockProxyServer } from "@test/support/mock.proxy.server";
import { createCategoryHandler } from "@src/gen";

const toResponse =
	({ status, ...body }: ResponseConfig) =>
	() =>
		new Response(JSON.stringify(body), {
			status: status === "SUCCESS" ? 200 : 500,
			headers: {
				"Content-Type": "application/json",
			},
		});

autoBindCreateCategorySteps(
	[
		({ given, then, when }) => {
			let page: CreateCategoryPageController;
			let driver: Driver;
			let mockServer: MockProxyServer;

			beforeAll(async () => {
				driver = await Driver.create({
					headless: false,
				});
				page = new PuppeteerCreateCategoryPageController(driver);
				mockServer = new MockProxyServer();
				mockServer.start();
			});

			beforeEach(async () => {
				await page.open();
			});

			given("I am a user", () => {});

			const successResponse = {
				status: "SUCCESS",
				message: "Success",
			} as const;

			when("I create a category with valid category details", async () => {
				await page.enterCategoryDetails({
					name: "Category",
					description: "Description1",
				});

				mockServer.mockResponse(
					createCategoryHandler(toResponse(successResponse)),
				);

				await page.submitForm();
			});

			then("I should see a success message", async () => {
				expect(await page.showsSuccessToastWithMessage(successResponse)).toBe(
					true,
				);
			});

			const invalidDetailsErrorResult = {
				status: "ERROR",
				code: "BODY_VALIDATION_EXCEPTION",
				errors: [
					{
						path: "name",
						message: "Invalid name",
					},
				],
				message: "Invalid",
			} as const;

			when("I register with invalid category details", async () => {
				mockServer.mockResponse(
					createCategoryHandler(toResponse(invalidDetailsErrorResult)),
				);

				await page.submitForm();
			});

			then(
				"I should see an error notifying me that my input is invalid",
				async () => {
					expect(await page.hasNameFormError()).toBe(true);
					expect(await page.hasDescriptionFormError()).toBe(false);
				},
			);

			const alreadyCreatedErrorResult = {
				status: "ERROR",
				message: "Category Already Created",
			} as const;

			given(/^a already created category with name: (.*)$/, () => {
				mockServer.mockResponse(
					createCategoryHandler(toResponse(alreadyCreatedErrorResult)),
				);
			});

			when(/^I attempt to create a category with name: (.*)$/, async (arg0) => {
				const name = CategoryName.parse(arg0);

				await page.enterCategoryDetails({ name });
				await page.submitForm();
			});

			then(
				"I should see an error for each category notifying me that the category already exists",
				async () => {
					expect(
						await page.showsErrorToastWithMessage(alreadyCreatedErrorResult),
					).toBe(true);
				},
			);
		},
	],
	{
		tagFilter: "@frontend",
	},
);
